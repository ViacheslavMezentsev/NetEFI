...
namespace cstools
{
    ...
    // 1. Describe the function using the Computable attribute.
    ...
    public class csimplot2d: CustomFunction<Complex, Complex, Complex, Complex, Complex[,], Complex[,]>
    {
        ...
        // 3. Implement the strictly-typed Execute method.
        public override Complex[,] Execute( Complex dxC, Complex dyC, Complex xminC, Complex yminC, Complex[,] matrix, Context context )
        {
            // Extract real values from complex inputs
            double dx = dxC.Real;
            double dy = dyC.Real;
            double xmin = xminC.Real;
            double ymin = yminC.Real;

            var rows = matrix.GetLength(0);
            var cols = matrix.GetLength(1);

            // Convert complex matrix to a double[,] for the algorithm
